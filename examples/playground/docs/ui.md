...
- The Knowledge scenario gets a **Corpus** card for each corpus, with **Delete** on each document and **Destroy the corpus**. The **Passages of a search** card shows the Passages of the Retriever in a `pre`. The **Ingest a document** card is a form with chips, and the `ingestForm` object keeps what the operator typed. The **Bulk ingest Job** card shows the progress of the Job, and the `watchJob` function reads the state on a timer while the Job is pending.
