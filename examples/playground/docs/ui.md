...
| `.walkthrough` | The numbered steps of a terminal walkthrough. It takes the place of the conversation, the composer and the side column. Each step is a `.card` with the commands in a `pre`, the exact output in a `pre` when the output does not change, and the expected result. |
...
- The Compaction and recovery scenario gets a **Ledger system** card with **Hold the ledger** and **Release the ledger**, and a **Context of the Agent** card with **Compact the Thread**. A panel action keeps the Thread of the conversation. Only a saved Agent Spec starts a new one. While the ledger is held and the Turn runs, the `watchProgress` function reads the state each second, because the held Tool call writes its entry without a Thread event.
...
- The **Feature coverage** page has no zones. It has a **Scenarios** `.table` with the setup, the starting data, the prerequisites and the local limits of each scenario, and a **Features** `.table` with one row for each feature. An empty cell says what it lacks, because a phone shows no header row.
- A scenario with `walkthrough` has no Agent. Its view is the title bar and one `.walkthrough` list. It has no composer, no side column and no **Reset scenario**.
- The transports scenario gets an **Event stream of this page** card and a **REST requests** card. A `select` in the stream card selects SSE or WebSocket, and **Drop the stream** and **Connect again** use the `setAttached` function. The stream card shows the last seq of the page and, after a reconnect, the seq range that the stream sent since the reconnect. The `seqLine` and `replayLine` nodes change on each event, thus the side column does not render again for each event. With WebSocket selected and the socket open, **Run** sends a `send` frame, and the card shows the answer frame. The **REST requests** card sends each request with `fetch`, not with the `api` function, because a request without the token must show its 401 answer. The `requestForm` object keeps the selected request and the last answer.
