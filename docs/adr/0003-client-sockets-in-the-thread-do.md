...
- Hibernation-safety is now a property that must hold: per-socket state lives in the socket's attachment, never in an instance field. It is tested by aborting the Durable Object instance while a socket is attached and asserting the socket still receives the next Turn's events, because hibernation itself cannot be forced in tests.
