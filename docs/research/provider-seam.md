...
4. **AI SDK stream shape for Anthropic-only blocks — RESOLVED.** In `@ai-sdk/anthropic` 4.0.45, compaction is tagged text, MCP use/result are typed provider-executed tool parts, fallback is absent from typed content but present in `iterations`, and `include.rawChunks` emits every original event before validation. See `docs/research/provider-stream-gateway-spike.md`.
5. **`anthropic-beta` passthrough on Cloudflare AI Gateway's native route — RESOLVED.** A live beta-gated `fallbacks: "default"` request returned HTTP 200 through the native route; the header is forwarded. See `docs/research/provider-stream-gateway-spike.md`.
