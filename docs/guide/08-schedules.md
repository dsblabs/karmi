...
- A `channelRef.deliverer` on the Event becomes the Deliverer of the Thread when the Schedule fires. `schedule` rejects an Event that names an unknown Deliverer with `deliverer.notFound`, and with `bindings.missing` when the Worker has no `KARMI_QUEUE` binding. [Threads](./04-threads.md#offline-delivery) describes offline delivery.
