...
beforeEach(() => { deliveries.length = 0; deliveryFailure.remaining = 0; });
...
it("delivers an offline payment webhook reply through the Queue", async () => {
  ...
  await thread.send({ kind: "event", type: "payment.received", payload: { amount: 100 }, channelRef: { deliverer: { name: "receipt", ref: { chat: "payer" } } } });
  ...
  expect(deliveries[0]!.events).toContainEvent({ type: "turn.completed", message: [{ type: "text", text: "Payment received" }] });
...
it("suppresses a completed reply consumed by a subscriber, then delivers a later offline Turn using the saved route", async () => {
  ...
  await thread.send({ kind: "message", parts: [{ type: "text", text: "Hello" }], channelRef: { deliverer: { name: "receipt", ref: "saved" } } });
  await clock.advance(1000);
...
it("delivers offline Approvals and the completion as separate ranges", async () => {
  ...
  await thread.send({ kind: "event", type: "booking.requested", payload: {}, channelRef: { deliverer: { name: "receipt", ref: "approver" } } });
...
it("retries a failed delivery without failing its Turn and no-ops queued work after Scope destruction", async () => {
  ...
  await thread.send({ kind: "event", type: "payment.received", payload: {}, channelRef: { deliverer: { name: "receipt", ref: "retry" } } });
  await expect.poll(async () => (await thread.events()).some(e => e.type === "turn.completed")).toBe(true);
  const events = await thread.events();
  const body = { kind: "delivery", scope: tenant.id, threadKey: thread.key, fromSeq: events[0]!.seq, toSeq: events.at(-1)!.seq };
  const consume = async () => {
...
it("delivers after a subscriber stops before consuming the completion", async () => {
  ...
  await thread.send({ kind: "event", type: "payment.received", payload: {}, channelRef: { deliverer: { name: "receipt", ref: "disconnected" } } });
  await expect.poll(async () => (await thread.events()).some(e => e.type === "turn.completed")).toBe(true);
...
it("captures each pending delivery's route before the next inbound input changes it", async () => {
  ...
  await thread.send({ kind: "event", type: "payment.received", payload: {}, channelRef: { deliverer: { name: "receipt", ref: "first" } } });
  await expect.poll(async () => (await thread.events()).some(e => e.type === "turn.completed")).toBe(true);
  await thread.send({ kind: "event", type: "payment.received", payload: {}, channelRef: { deliverer: { name: "receipt", ref: "second" } } });
  await expect.poll(async () => (await thread.events()).filter(e => e.type === "turn.completed").length).toBe(2);
  await clock.advance(1000);
  await expect.poll(() => deliveries.length).toBe(2);
  expect(deliveries.map(d => ({ ref: d.ref, turn: d.events[0]!.turn })).sort((a, b) => a.turn - b.turn)).toEqual([{ ref: "first", turn: 1 }, { ref: "second", turn: 2 }]);
});
...
it("rejects an unknown delivery route before accepting the input", async () => {
  ...
  await expect(thread.send({ kind: "event", type: "payment.received", payload: {}, channelRef: { deliverer: { name: "missing", ref: "payer" } } })).rejects.toMatchObject({ code: "deliverer.notFound" });
  expect(await thread.events()).toEqual([]);
});


it.each([
  { name: "receipt-parts", types: ["turn.started", "step.started", "message.part", "step.completed", "turn.completed"] },
  { name: "receipt-deltas", types: ["turn.started", "step.started", "message.delta", "message.part", "step.completed", "turn.completed"] },
  ...
  await thread.send({ kind: "event", type: "payment.received", payload: {}, channelRef: { deliverer: { name, ref: "payer" } } });
  await expect.poll(async () => (await thread.events()).some(e => e.type === "turn.completed")).toBe(true);
  await clock.advance(1000);
  await expect.poll(() => deliveries.length).toBe(1);
