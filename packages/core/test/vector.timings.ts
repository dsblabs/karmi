...
it("measures bounded SQLite vector scans in workerd", async () => {
  ...
  for (const count of [1000, 10000]) {
    await runInDurableObject(stub, async (_, state) => {
      const db = openKnowledgeDatabase(state.storage.sql);
      const store = createSqliteBruteForceStore(db, index);
    ...
    for (const page of [1000, 10000]) {
      ...
      for (let repeat = 0; repeat < 6; repeat++) {
        ...
        await runInDurableObject(stub, async (_, state) => {
          const store = createSqliteBruteForceStore(openKnowledgeDatabase(state.storage.sql), index, page);
