...
import { normalizeToolCallId, prepareMessages, type ContentBlock, type Message, type StopReason } from "../src/index.js";

const claude = { provider: "anthropic", model: "claude-sonnet-5" };
const gpt = { provider: "openai", model: "gpt-5" };

const assistant = (content: ContentBlock[], from = claude, stopReason: StopReason = "end_turn"): Message => ({ role: "assistant", ...from, content, stopReason });
const user = (text: string): Message => ({ role: "user", content: [{ type: "text", text }] });
const result = (toolCallId: string, text = "ok"): Message => ({ role: "toolResult", toolCallId, toolName: "weather", content: [{ type: "text", text }], isError: false });
const call = (id: string): ContentBlock => ({ type: "tool_call", id, name: "weather", input: { city: "Oslo" } });

describe("prepareMessages", () => {
  describe("thinking retention", () => {
    const signed = assistant([{ type: "thinking", text: "let me see", signature: "sig" }, { type: "text", text: "Hi" }]);
    const redacted = assistant([{ type: "thinking", text: "", redacted: true }, { type: "text", text: "Hi" }]);
    const unsigned = assistant([{ type: "thinking", text: "let me see" }, { type: "text", text: "Hi" }]);
    ...
    it("turns thinking into text for another model and drops what cannot be read", () => {
      expect(prepareMessages([user("q"), signed], gpt).messages[1]).toMatchObject({ content: [{ type: "text", text: "let me see" }, { type: "text", text: "Hi" }] });
      expect(prepareMessages([user("q"), redacted], gpt).messages[1]).toMatchObject({ content: [{ type: "text", text: "Hi" }] });
      expect(prepareMessages([user("q"), unsigned], { ...claude, model: "claude-opus-5" }).messages[1]).toMatchObject({ content: [{ type: "text", text: "let me see" }, { type: "text", text: "Hi" }] });
    });

    it("drops empty unsigned thinking even for the same model", () => {
      const empty = assistant([{ type: "thinking", text: "  " }, { type: "text", text: "Hi" }]);
      expect(prepareMessages([user("q"), empty], claude).messages[1]).toMatchObject({ content: [{ type: "text", text: "Hi" }] });
    });
  });
  ...
  describe("tool-call ids", () => {
    ...
    it("keeps a thought signature only for the same model", () => {
      ...
      const signed = assistant([{ type: "tool_call", id: "c1", name: "weather", input: { city: "Oslo" }, signature: "thought" }], gemini);
      expect(prepareMessages([user("q"), signed, result("c1")], gemini).messages[1]).toEqual(signed);
      expect((prepareMessages([user("q"), signed, result("c1")], claude).messages[1] as { content: object[] }).content[0]).toEqual(call("c1"));
    });
  });

  describe("orphaned tool calls", () => {
    it("synthesises an error result for every call without one, before the next message", () => {
      const messages = prepareMessages([user("q"), assistant([call("c1"), call("c2")]), result("c2"), user("next")], claude).messages;
      expect(messages.map((m) => m.role)).toEqual(["user", "assistant", "toolResult", "toolResult", "user"]);
      expect(messages[3]).toEqual({ role: "toolResult", toolCallId: "c1", toolName: "weather", content: [{ type: "text", text: "No result provided" }], isError: true });
    });
  ...
  describe("system placement", () => {
    it("hoists leading system messages into the request system and merges neighbours", () => {
      const prepared = prepareMessages([{ role: "system", content: "A" }, { role: "system", content: "B" }, user("q"), assistant([{ type: "text", text: "Hi" }]), { role: "system", content: "C" }, { role: "system", content: "D" }, user("more")], claude);
    ...
    it("never splits a tool call from its results", () => {
      const prepared = prepareMessages([user("q"), assistant([call("c1")]), { role: "system", content: "steer" }, result("c1"), user("more")], claude);
  ...
  describe("provider-opaque blocks", () => {
    const search: Message = assistant([{ type: "server_tool", id: "s1", name: "web_search", input: { q: "x" }, result: { raw: { encrypted: "…" }, summary: "3 results about x" } }, { type: "text", text: "Found it" }]);
    const compacted: Message = assistant([{ type: "compaction", summary: "Earlier we discussed x.", raw: { type: "compaction", content: "…" } }]);
    const foreign: Message = assistant([{ type: "provider", raw: { type: "fallback" } }, { type: "text", text: "Hi" }]);

    it("replays them byte-exact to the same provider, even on another model", () => {
      const other = { ...claude, model: "claude-opus-5" };
      expect(prepareMessages([user("q"), search, compacted, foreign], other).messages.slice(1)).toEqual([search, compacted, foreign]);
    });
    ...
    it("hands another provider the summaries and drops what has none", () => {
      ...
      expect(a).toMatchObject({ content: [{ type: "text", text: "3 results about x" }, { type: "text", text: "Found it" }] });
  ...
  it("passes text, media and tool results through untouched and stays JSON-serialisable", () => {
    const messages: Message[] = [user("q"), { role: "user", content: [{ type: "media", media: { id: "m1", key: "s/media/t/m1", mimeType: "image/png", bytes: 10 } }] }, assistant([call("c1")]), result("c1")];
