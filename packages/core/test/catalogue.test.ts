...
const greeting = defineFragment({
  ...
  args: z.object({ tone: z.string() }),
  render: (_ctx, { tone }) => `Be ${tone}.`,
});
const research = defineSkill({
  name: "research",
  description: "Deep research",
  body: () => "Steps…",
  tools: [defineTool({ name: "search", description: "Search", input: z.object({ q: z.string() }), execute: () => "" })],
});
...
const concierge = defineAgent({
  ...
  tools: ["weather"],
  connections: { weather_api: { type: "api_key", level: "agent" } },
});

describe("assembleCatalogue", () => {
  it("rejects duplicate names within a kind", () => {
    expect(() => assembleCatalogue({ tools: [weather, weather] })).toThrowError(
      new KarmiError("name.duplicate", 'Duplicate tool name "weather" in the Catalogue.'),
    );
  ...
  it("treats a Skill's Tools as Tools for uniqueness", () => {
    ...
    expect(() => assembleCatalogue({ tools: [clash], skills: [research] })).toThrowError(
  ...
  it("allows the same name across kinds", () => {
    const catalogue = assembleCatalogue({
      tools: [weather],
      fragments: [defineFragment({ name: "weather", render: () => "" })],
    });
  ...
  it("describes every item as JSON with schemas", () => {
    const description = assembleCatalogue({
      tools: [weather],
      fragments: [greeting],
      skills: [research],
      retrievers: [fts],
