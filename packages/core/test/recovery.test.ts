...
it("recovers an evicted model Step on the clock's watchdog without rotating models", async () => {
  ...
  await karmi.scope("test").thread({ agent: "concierge", threadId }).send({ kind: "message", parts: [{ type: "text", text: "Go" }] });
...
for (const name of ["recover_read", "recover_idempotent", "recover_mutation"]) {
  it(`recovers ${name} without replaying completed calls or before-tool hooks`, async () => {
    ...
    provider.script([[reply.toolCall(name, { id: "finished" }, "a"), reply.toolCall(name, { id: "unfinished" }, "b")], "Recovered tools"]);
    const threadId = name;
    const target = { agent: "recovery", threadId };
    const thread = scope.thread(target);
    await karmi.scope("test").thread(target).send({ kind: "message", parts: [{ type: "text", text: "Go" }] });
    ...
    if (name === "recover_mutation") {
      ...
      expect(events).toContainEvent({ type: "tool.result", id: "b", isError: true, interrupted: { attempt: 2 }, content: [{ type: "text", text: "This call was interrupted before it reported a result. It may or may not have taken effect; check before repeating it." }] });
      expect(recovery.after[1]).toMatchObject({ isError: true, interrupted: { attempt: 2 } });
    } else {
      expect(calls).toEqual([{ id: "finished", callId: expect.any(String), attempt: 1 }, { id: "unfinished", callId: calls[1]!.callId, attempt: 1 }, { id: "unfinished", callId: calls[1]!.callId, attempt: 2 }]);
...
it("fails after three attempts at one Step and drains the queued input", async () => {
  ...
  await expect.poll(async () => (await thread.events()).some(e => e.turn === 2 && e.type === "turn.completed")).toBe(true);
  const events = await thread.events();
  expect(events).toContainEvent({ type: "turn.failed", turn: 1, reason: "recovery" });
  expect(events.filter(e => e.turn === 1).filter(e => e.type === "step.started").map(e => e.attempt)).toEqual([1, 2, 3]);
...
it("resets the recovery budget at each completed Step", async () => {
  ...
  await clock.advance("1m");
  await expect.poll(async () => (await thread.events()).some(e => e.type === "turn.completed")).toBe(true);
  expect((await thread.events()).filter(e => e.type === "step.started").map(e => [e.n, e.attempt])).toEqual([[1, 1], [1, 2], [2, 1], [2, 2], [3, 1], [3, 2]]);
});

it("bounds a model Step to three attempts even when it first falls back", async () => {
  provider.script(({ index }) => index === 0 ? [reply.error({ code: "unavailable" })] : new Promise<string>(() => {}));
