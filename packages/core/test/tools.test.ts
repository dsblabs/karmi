...
describe("tool Step", () => {
  it("runs a read-only batch in parallel as one tool Step, then calls the model again with the results", async () => {
    provider.script([
      [reply.toolCall("lookup", { id: "a" }, "c1"), reply.toolCall("lookup", { id: "b" }, "c2")],
      "Both found",
    ]);
    ...
    expect(events).toContainEvent({
      ...
      id: "c1",
      name: "lookup",
      content: [{ type: "text", text: "Guest a" }],
      isError: false,
    });
    ...
    expect(trace).toEqual([
      "before-turn:1:message",
      "lookup:start:a",
      "lookup:start:b",
      "lookup:end:a",
      "after-tool:lookup:ok",
      "lookup:end:b",
      "after-tool:lookup:ok",
      "after-turn:1:turn.completed",
    ]);
    ...
    expect(provider.requests[1]?.messages).toEqual([
      { role: "user", content: [{ type: "text", text: "Find a and b" }] },
      {
        role: "assistant",
        content: [
          { type: "tool_call", id: "c1", name: "lookup", input: { id: "a" } },
          { type: "tool_call", id: "c2", name: "lookup", input: { id: "b" } },
        ],
        provider: "fake",
      ...
      },
      {
        ...
        toolCallId: "c2",
        toolName: "lookup",
        content: [{ type: "text", text: "Guest b" }],
        isError: false,
      },
    ]);
  });
...
describe("batch sequencing", () => {
  it("runs a mutating Tool alone, before and after read-only calls of the same batch", async () => {
    provider.script([
      [
        reply.toolCall("lookup", { id: "a" }, "c1"),
        reply.toolCall("book", { room: 7 }, "c2"),
        reply.toolCall("lookup", { id: "b" }, "c3"),
        reply.toolCall("book", { room: 8 }, "c4"),
      ],
      "Done",
    ]);
    const events = await fresh().send(message("Book"));
    expect(trace.filter((entry) => !entry.startsWith("after-") && !entry.startsWith("before-"))).toEqual([
      "lookup:start:a",
      "lookup:end:a",
      "book:start:7",
      "book:end:7",
...
describe("Permission Policy", () => {
  it("never offers a denied Tool and answers a call to it with an isError result, without running it", async () => {
    ...
    expect(events).toContainEvent({
      ...
      name: "book",
      isError: true,
      content: [{ type: "text", text: 'Tool "book" is denied by the Permission Policy.' }],
    });
    expect(trace).toEqual([]);
...
describe("Hooks", () => {
  ...
  it("lets a before-tool Hook deny a call the Policy allowed", async () => {
    ...
    expect(events).toContainEvent({
      ...
      id: "c1",
      isError: true,
      content: [{ type: "text", text: 'Tool "lookup" was refused by a Hook: Not now' }],
    });
    expect(trace).toEqual([]);
  });
  ...
  it("dispatches before-turn, after-tool, after-turn and on-error by name with the Turn's context", async () => {
    ...
    expect(trace).toEqual([
      "before-turn:1:message",
      "after-tool:failing:error",
      "on-error:provider",
      "after-turn:1:turn.failed",
    ]);
  });
});
...
describe("Tool context", () => {
  it("carries callId, attempt, validated settings and the agent-level Connection", async () => {
    ...
    expect(seen).toEqual({
      ...
      connection: { name: "crm", type: "crm", level: "agent", value: { token: "t-1" } },
      attempt: 1,
      callId: `tool-${n}:${call.seq}`,
      aborted: false,
    });
  ...
  it("answers a call whose Connection is missing with an isError result", async () => {
    ...
    expect(events).toContainEvent({
      ...
      id: "c1",
      isError: true,
      content: [{ type: "text", text: 'Connection "crm" is not available.' }],
    });
  });
  ...
  it("rejects input that fails the Tool's schema before running it", async () => {
    ...
    expect((events.find((e) => e.type === "tool.result") as { content: { text: string }[] }).content[0]!.text).toMatch(
      /^Invalid input for "book": room:/,
    );
  ...
  it("answers an unknown Tool name with an isError result", async () => {
    ...
    expect(events).toContainEvent({
      ...
      name: "teleport",
      isError: true,
      content: [{ type: "text", text: 'Unknown tool "teleport".' }],
    });
  });
});
...
describe("Spill", () => {
  it("stores an oversized result whole, shows head + tail + marker, and read_output pages it back", async () => {
    ...
    expect(events).toContainEvent({
      ...
      name: "read_output",
      isError: false,
      content: [{ type: "text", text: "line 51\nline 52\nline 53" }],
    });
  ...
  it("refuses to read an output that is not on this Thread", async () => {
    ...
    expect(events).toContainEvent({
      ...
      id: "c1",
      isError: true,
      content: [{ type: "text", text: 'No stored output "999" on this Thread.' }],
    });
  });
});
