...
`before-compact { trigger, instructions?, tokensBefore }` Hooks may answer `{ skip: true }` or `{ summary }`; a Hook summary is ignored under the `provider` strategy, which cannot take one. `after-compact { compacted }` observes. `thread.compact({ instructions })` compacts an idle Thread on request. `thread.fork(seq, { threadId? })` opens a new Thread for the same Agent and User holding the log up to `seq`, after a Compaction included; the media that log refers to, spilled Tool output included, is copied into the fork before it resolves, so the fork keeps it after the original is deleted. A failed copy rejects the fork and leaves no Thread behind. The fork joins the Scope's Thread index on its first Turn.
