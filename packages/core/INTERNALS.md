...
| Thread and Turn   | `thread-do.ts`, `event-log.ts`, `turn-state.ts`, `thread-events.ts`, `transcript.ts`, `compaction.ts`, `fork.ts`                                           | The Thread Durable Object, the event log and the Turn loop.                                     |
...
| Scripts           | `sandbox.ts`, `isolate-sandbox.ts`, `container-sandbox.ts`, `cloudflare-container.ts`, `scripts.ts`                                        | The `Sandbox` interface and its isolate and container adapters.                                 |
...
- A module that owns Thread tables takes only the database in its constructor. `openThreadDatabase` in `db/thread/database.ts` opens that database for a direct test. `test/event-log.test.ts`, `test/usage-outbox.test.ts` and `test/delivery-outbox.test.ts` are examples. Such a test contains only the rules of the module. The Turn-level tests show that the modules work together.
...
- `DeliveryOutbox` in `deliverer.ts` owns the `delivery_route` and `deliveries` tables. It calculates the event range of each delivery. The Thread Durable Object sets the `delivery` alarm and sends the range to the Queue.
