...
- `DeliveryOutbox` in `deliverer.ts` owns the `delivery_route` and `deliveries` tables. It calculates the event range of each delivery. The Thread Durable Object sets the `delivery` alarm and sends the range and its route to the Queue. After the Queue accepts the message, or skips because a Subscriber is attached, the Durable Object drops the ranges of a Turn that has ended. It sets the route from the `channelRef` of an input in `send` and when a Schedule fires.
