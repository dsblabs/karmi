...
- A module that owns Thread tables takes only the database in its constructor. `openThreadDatabase` in `db/thread/database.ts` opens that database for a direct test. `test/event-log.test.ts` is the example. Such a test contains only the rules of the module. The Turn-level tests show that the modules work together.
