...
- `ThreadWorkspace` owns `container_run`, `container_workspace`, the `container-idle` and `container-watchdog` alarms and the Scope container slot. The Thread decides when a Turn ends and calls `destroy()`. A failed or slow container destroy does not stop the end of the Turn: `destroy()` keeps `container_workspace` and the Scope slot, and the `container-idle` alarm tries again. The cleanup of a deleted Thread waits for a successful destroy before it empties the tables, thus it never loses the Scope slot. The Workspace records the end of a Job in the same write that clears `container_run`. The delete of a Thread also empties `alarms`.
- Each `tool.call` in the log gets exactly one `tool.result`, a nested call of a Script included. `interruptedResults` in `tool-step.ts` finds the calls without a result. `cancelTurn` gives each one an interrupted result and then logs `step.completed` for the tool Step. A tool Step that runs again gives one to each nested call, because a Script does not survive an eviction. After the Turn signal aborts, the `append` of a tool Step throws, thus a Tool that returns late cannot log a second result. The after-tool Hooks do not run for the results that a cancel writes.
...
- The Scope config Durable Object never stores the code definition of a code-defined Agent. `readAgent` returns it as version 0 when the Scope has no Override. The set of Agents that `validate` checks against is the stored Agents in use, and the code-defined Agents without an Override. A Turn revalidates only a stored Spec whose Catalogue fingerprint changed. The Turn snapshot applies the Scope ceilings to each Spec ([ADR 0007](../../docs/adr/0007-code-defined-agents-follow-the-code.md)).
