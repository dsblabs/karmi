...
- `DeliveryOutbox` in `deliverer.ts` owns the `delivery_route` and `deliveries` tables. It calculates the event range of each delivery. The Thread Durable Object sets the `delivery` alarm and sends the range to the Queue. It sets the route from the `channelRef` of an input in `send` and when a Schedule fires.
...
- The Thread Durable Object queries only the tables that have no module: `thread`, `deleted`, `container_run` and `container_workspace`. The delete of a Thread also empties `alarms`.
