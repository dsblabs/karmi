...
| Thread and Turn   | `thread-do.ts`, `turn-state.ts`, `thread-events.ts`, `transcript.ts`, `compaction.ts`, `fork.ts`                                           | The Thread Durable Object, the event log and the Turn loop.                                     |
