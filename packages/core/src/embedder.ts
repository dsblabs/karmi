...
import { validateVector, type Embedder, type EmbeddingIndex } from "./vector-store";
const responseSchema = z.object({ data: z.array(z.array(z.number())) });
/** The Workers AI embedding configuration used when no Embedder is supplied. */
export const defaultEmbeddingIndex = Object.freeze({
  ...
  return {
    ...
    async embed(texts) {
      ...
      for (let offset = 0; offset < texts.length; offset += 100) {
        ...
        const response = z.parse(responseSchema, await ai.run(defaultEmbeddingIndex.model, { text: batch }));
        ...
        for (const values of response.data) {
          ...
          validateVector(vector, defaultEmbeddingIndex.dims);
          vectors.push(vector);
        }
      }
      return vectors;
