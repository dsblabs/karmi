import type { ApprovalAnswer, Granularity, TurnInput } from "./thread-events";
import * as z from "zod/mini";
/** A malformed Thread wire request, retaining the published transport error code. */
...
function parse<S extends z.ZodMiniType>(schema: S, value: unknown, what: string): z.output<S> {
  ...
  throw new ThreadProtocolError(`Invalid ${what}${path ? ` at ${path}` : ""}: ${issue?.message ?? "malformed"}.`);
}
...
/** Decodes a JSON Turn body: a `TurnInput` with an optional `steer` flag. Throws `ThreadProtocolError` for anything else. */
...
/** Parses JSON text a client sent as `what`. Throws `ThreadProtocolError` when it is not JSON. */
export function parseJsonText(text: string, what: string): unknown {
  try {
    return JSON.parse(text);
  } catch {
    throw new ThreadProtocolError(`The ${what} is not valid JSON.`);
  }
}

/** Decodes one WebSocket frame from its JSON text. Throws `ThreadProtocolError` for anything else. */
...
export type ServerFrame =
  | import("./thread-events").ThreadEvent
  /** The frame with `id` succeeded. Void calls return null. */
  | { type: "ack"; id?: string | number; result: unknown }
  /** The frame failed, or could not be decoded when `id` is absent. */
