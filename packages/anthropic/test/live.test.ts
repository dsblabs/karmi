...
describe.skipIf(!env.ANTHROPIC_API_KEY)("live Anthropic", () => {
  it("streams one short reply and reports usage", async () => {
    ...
    const events = await collect(provider, request({ model: "claude-haiku-4-5", params: { maxOutputTokens: 16, reasoning: "off" }, messages: [{ role: "user", content: [{ type: "text", text: "Reply with the single word: ok" }] }] }), { fetch: scopedFetch({ hosts: ["api.anthropic.com"] }) });
    ...
    expect(events.at(-1)).toMatchObject({ type: "message.end", usage: { input: expect.any(Number), output: expect.any(Number) } });
  }, 30_000);
});
